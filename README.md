...
- Превью пока не понятно как делать (яндекс диск поддерживает создание превью? https://github.com/jack-theripper/yandex/blob/master/example/manual_request.php#L20)
...
- Использовать api яндекса (фоотки, диск, datasync, market ???)
